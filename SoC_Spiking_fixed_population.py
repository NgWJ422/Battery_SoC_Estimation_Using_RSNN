...
from bindsnet.encoding.encodings import poisson_normalized
from my_encoding import fix_normalized, latency_encoding
...
#================== Parameter Setting =======================
n_features = 70                  # Number of features
n_neurons = 500                # Number of neuron in reservoir network
n_layers = 3                    # Number of hidden layers
n_hidden = 100         # Number of neurons in each hidden layer in regression model
...
time = 50                   # window time. conversion time window and spike rate (that represent value)
dt = 1.0                    # simulation timestep
max_rate = 25               # max spiking rate each neuron during conversion
...
resConn_density = 0.3      # Connection density within nodes in reservoir network
                            # set range between [0~1]. 1:all connected; 0.5:averagely 50% connections; 0:not connected
inpConn_weight  = 2       # Max value connection weight between input and reservoir network
resConn_weight = 2        # Max value connection weight between nodes in reservoir network
                            # The weight also depend on this parameter "inpConn_type"/"resConn_type"
inpConn_type = ['rand','inb', 0.3]          # 1st argument: set 'uni' for uniform weight, set 'rand' for random weight
...
def getSpikeTime(spikes):
    spike_tensor = spikes["O"].get("s")  # Shape: [T, 1, N]
    spike_tensor_debug = spike_tensor.squeeze(1).numpy()

    # Time indices for each timestep
    time_indices = torch.arange(spike_tensor.size(0), device=spike_tensor.device).view(-1, 1, 1)  # Shape: [T, 1, 1]

    # Replace False with max time (T) and keep real spike time
    masked_times = torch.where(spike_tensor, time_indices, torch.full_like(time_indices, time))

    # First spike time across time axis (dim=0), result shape: [1, N]
    first_spike_time = masked_times.min(dim=0).values

    # Normalize: early spike → higher value, no spike → 0
    #data = (time - first_spike_time).float() / time
    #data = torch.clamp(normalized, min=0.0, max=1.0)  # Ensure values in [0, 1]
    data = first_spike_time.float()

    return data


def gaussianPopulation(input_tensor, n_pop=10, input_range=(0.0, 1.0), sigma=0.1):
    # input_tensor: shape [batch, features] — e.g., [1, 7]
    batch_size, n_features = input_tensor.shape
    centers = torch.linspace(*input_range, steps=n_pop, device=input_tensor.device)  # [n_pop]

    # Expand input and centers for broadcasting
    input_expanded = input_tensor.unsqueeze(-1)  # [batch, features, 1]
    centers_expanded = centers.view(1, 1, n_pop)  # [1, 1, n_pop]

    # Gaussian activation
    responses = torch.exp(-0.5 * ((input_expanded - centers_expanded) / sigma) ** 2)  # [batch, features, n_pop]

    # Flatten the last two dims: [batch, features * n_pop]
    responses = responses.view(batch_size, n_features * n_pop)
    return responses
...
def trainNetwork(network,train_dataset,device):
    ...
    for (i, dataPoint) in pbar:
        ...
        #datum = dataPoint[0].float()                                       # for other encoding technique
        datum = gaussianPopulation(dataPoint[0].float(), n_pop=10)         # for population encoding technique
        target = dataPoint[1].float()


        # ---------------------------------Different Encoder Techniques-----------------------------------------------#

        # encoded_datum = poisson_normalized(datum, maxrate=max_rate, avgISI=avg_ISI, time=time, dt=dt)
        encoded_datum = fix_normalized(datum, maxrate=max_rate, time=time, dt=dt)
        #encoded_datum = latency_encoding(datum, time=time, dt=dt)

        # encoded_datum = encoded_datum.to(dtype=eval(f"torch.{datatype}"))   #KIV - this cause low accuracy. The idea is to low the memory usage
        ...
        spikedata = spikes["O"].get("s").sum(0).float()                             #if record spike rate
        #spikedata = getSpikeTime(spikes)                                   #if record spike time

        #spikedata = spikedata.to(dtype=eval(f"torch.{datatype}"))                  #to convert spike data as int type to save memory
...
def testNetwork(network,model,test_dataset,device):
    ...
    for (i, dataPoint) in pbar:
        if  test_all is False and i > test_samples:  # increase example value to increase accuracy
            break
        cycle = dataPoint[0]
        # datum = dataPoint[1].float()                                       # for other encoding technique
        datum = gaussianPopulation(dataPoint[1].float(), n_pop=10)           # for population encoding technique
        ...
        testing_pairs.append([cycle, SoCtime, spikedata, target])
...
def performanceAnalysis(data):
    ...
    if record is True:
        ...
        with open('SoC_simulation_analysis.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(analysis_result)

        #record SoC result
        data.to_csv('SoC_Prediction_results.csv', index=False)
