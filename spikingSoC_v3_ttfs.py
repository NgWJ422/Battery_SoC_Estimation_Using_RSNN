...
from my_encoding import rate_encoding, ttfs_encoding, burst_encoding, phase_encoding, population_encoding, dynamic_threshold_encoding, ttfs2_encoding
...
train_all = False          # set "True" for use all data during training. set "False" with number of "examples"
test_all = False             # set "True" for use all data during testing. set "False" with number of "test_samples"
...
def createNetModel(device):
    ...
    output = LIFNodes(n_neurons, thresh=thresh, refrac=trefrac, tc_decay=tdecay)
...
def trainNetwork(network,train_dataset,device):
    ...
    for (i, dataPoint) in pbar:
        ...
        encoded_datum = ttfs2_encoding(datum, time, dt, device, debug=False)
        ...
        spike_tensor = spikes["O"].get("s")
        if spike_tensor.sum().item() == 0:
            print(f"WARN: No spikes from reservoir for sample {i}!")
        
        
        fallback_time = torch.full((spike_tensor.shape[1],), float(time), device=spike_tensor.device)
        first_spike_times = torch.where(
            spike_tensor.any(dim=0),
            (spike_tensor > 0).float().argmax(dim=0).float(),
            fallback_time
        )
        ...
        # Store training pair
        training_pairs.append([first_spike_times, target])
    ...
    for epoch, _ in pbar:
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch}, Sample Output: {outputs[:5].squeeze()}, Target: {t[:5].squeeze()}")
...
def testNetwork(network,model,test_dataset,device):
    ...
    for (i, dataPoint) in pbar:
        ...
        encoded_datum = ttfs2_encoding(datum, time, dt, device)
        ...
        # Run network on sample input
        network.run(inputs={"I": encoded_datum}, time=time, input_time_dim=1)

        spike_tensor = spikes["O"].get("s")  # shape: [time, neurons]
        if spike_tensor.sum().item() == 0:
            print(f"WARN: No spikes from reservoir for sample {i}!")
        ...
        testing_pairs.append([cycle, SoCtime, first_spike_times, target])

        # use debug to view testing pairs
        #testing_pairs_debug = testing_pairs[i][0].numpy()

        network.reset_state_variables()
    all_test_spike_times = torch.stack([s for _, _, s, _ in testing_pairs])
    print("Test feature variance:", all_test_spike_times.var(dim=0).mean())
...
def performanceAnalysis(data):
    ...
    save_dir = "SoC_Pred_Results_ttfs_test2"
...
def main():
    ...
    print(f"Sample {i+1} - Features: {features}, Label: {labels}")
