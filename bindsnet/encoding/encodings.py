...
def poisson_normalized(
    ...
    **kwargs,
    ...
    if approx:
        #not finish, later need to modified
        ...
        # Compute firing rates in time window as function of normalized input data
        firerate = torch.poisson(datum  * maxrate)      #
        # Initialize spike trains tensor
        spikes = torch.zeros(time, firerate.shape[0])


        # Generate spike trains based on Poisson distribution for intervals
        for i in range(datum.shape[0]):
            total_spikes = int(firerate[i].item())
            intervals = torch.zeros(total_spikes, dtype=torch.int32)
            for j in range(total_spikes):
                # Generate spike index based on Poisson distribution
                inter_spike_intervals = torch.poisson(torch.tensor(float(avgISI)))
                inter_spike_intervals = max(1, inter_spike_intervals)   #cap minimum value at 1
                # interval is not fix but randomly generated
                intervals[j] = inter_spike_intervals            # Add inter_spike_intervals to intervals
                spike_times = torch.cumsum(intervals, dim=0)    # accumulate the intervals to get time
                spike_times = (spike_times / dt).long()         # Convert to time step indices
                spike_times = spike_times[spike_times < time]   # Clip spike times within time window
...
def poisson(
    ...
    **kwargs,
    ...
    if approx:
        ...
        rate[datum != 0] = 1 / datum[datum != 0] * 100 * (1000 / dt)
